...
**v1（P0〜P7）・v2（P8〜P10）完了、Vercel にデプロイ済み。次は v3 の相談と計画から。**
...
- v2 の計画・決定・**v3 の候補**: [docs/plan-v2.md](docs/plan-v2.md)（末尾）
...
- **非収束は例外ではなく戻り値。「解けなかった」と「間に合わなかった」を区別する**:
  ...
  言ってよい）は別物。
- 「結合に寄与する電子」はエンジンが選ぶ（平面分子なら π、それ以外は符号付きの差密度）。
  UI は返ってきた `channel` で説明文と色を変える。
...
### UI
- **緩和の前に、平らで手で作った構造だけ揺らす**（`perturb.ts` 0.05 Å、App で
  `presetId === null && isFlat(xyz)`）。クリックは全原子をカメラ平面に置くので、そのまま
  最適化すると平面の鞍点で「収束」する（平面 CH₄ は正四面体より 834 kJ/mol 上）。プリセットや
  記録から開いた構造は揺らさない（揺らすとベンゼンが 3 歩 → 15 歩）。**探索の候補は別の振幅**
  （`candidates.ts` 0.2 Å）。
...
- **記録**（`hasUsableStructure()` で終わるたびに 1 件）。**比べてよい組は Hill 式 + 電荷 +
  `ENGINE_MODEL`**（電荷は `driver` が選ぶので内部の比較キーに使い、**画面には出さない**）。
  `ENGINE_MODEL` は基底・汎関数・最適化グリッドを変えたら書き換える。**同じ谷は 1 kJ/mol 以内**。
...
- **v1・v2 完了**、いずれも Firefox 確認済み。v2 で足したのは観測モード・安定構造の記録・
  並列探索（パネルの「形をさがす」）。**次は v3 の相談と計画**（plan-v2.md 末尾が出発点）。
- **v3 の筆頭候補は基底と汎関数の見直し**（6-31G\* + 汎関数）。理由は 2 つ: **幾何が教科書と
  数度ずれる**（水 96.7° / 実験 104.5°）、**異性体のエネルギー順序が逆に出る**（試した 2 組とも。
  配座の比較は合う）。どちらも最小基底のためでエンジンの誤りではなく、記録の節に注意書きを
  出してある（`ISOMER_CAVEAT`）。実測は dev-notes の該当 2 節。
- そのほかの候補（dev-notes「P6 以降に残したもの」）: XC グリッド重み微分、マルチスレッド化
  （nightly 依存）、`OPTIMIZE_BUDGET_SECONDS`（1800 秒）の見直し。**WebGPU は採らないと決めた**。
- **講義で使う PC**: Windows 11 / i5-1235U（P 2 + E 8、12 スレッド、15 W）/ Chrome。`poolSize` は
  3 になる。**その PC での実測はまだ無い**ので、重いようなら `MAX_CONCURRENT` を 2 に下げる。
