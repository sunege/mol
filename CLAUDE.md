...
- **v2 着手（2026-09-20）。** P8（観測モード）完了（Firefox 確認済み）。P9（安定構造の記録）は
  P9-1〜P9-8 の実装が完了し、**残りはユーザーの Firefox 確認**（plan-v2.md の P9 の節）。
...
- **異性体どうしのエネルギー比較は、このモデルでは当てにならない**（2026-09-20 実測）。試した
  2 組とも順序が逆: C₃H₆（アプリはシクロプロパンが 19.7 kJ/mol 低い / 実験はプロペンが 32.9 低い）、
  C₂H₆O（アプリはジメチルエーテルが 28.2 低い / 実験はエタノールが 49.9 低い。ΔfH は NIST）。
  符号を変えているのは基底（6-31G 以上なら正しい）だが、差の大きさには汎関数も効く。
  **一方、配座（つながり方が同じ）の比較は合う**（エタンの回転障壁 13.5 kJ/mol、教科書 約 12）
  ので、記録の使いどころはそちら。**記録の節に 1 行そう書いてある**（`ISOMER_CAVEAT`。
  基底・汎関数は出さない）。**v3 では基底と汎関数を一緒に見直す。**
  実測は dev-notes「異性体のエネルギー順序も最小基底で逆になる」。
